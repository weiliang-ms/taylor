...
func (f *CustomFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	...
	if f.isColored() {
		...
		f.appendKeyValue(b, "", "[taylor] "+hostName)
...
func (f *CustomFormatter) printColored(b *bytes.Buffer, entry *logrus.Entry, keys []string, data Fields, timestampFormat string) {
	var levelColor int
	switch entry.Level {
...
func (f *CustomFormatter) appendValue(b *bytes.Buffer, key string, value interface{}) {
...
func init() {
	...
	format := CustomFormatter{EnvironmentOverrideColors: true, TimestampFormat: "2006-01-02 15:04:05"}
